...
/**
 * Show skeleton placeholder items while files are being read.
 * @param {number} count - number of skeletons to show
 */
export function showUploadingSkeleton(count) {
  ...
  list.innerHTML = Array.from({ length: count }).map(() => `
    <div class="file-item skeleton-item">
      <div class="file-ext skeleton-box" style="width:36px;height:36px;border-radius:8px"></div>
      <div class="file-info">
        <div class="skeleton-box" style="width:55%;height:13px;border-radius:4px;margin-bottom:6px"></div>
        <div class="skeleton-box" style="width:30%;height:11px;border-radius:4px"></div>
      </div>
